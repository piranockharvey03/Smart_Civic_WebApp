...
function admin_settings_defaults(): array
{
    return [
        ...
        'logo_url' => '',
        'default_statuses' => json_encode(issue_status_catalog(), JSON_UNESCAPED_SLASHES),
        'default_priorities' => json_encode(issue_priority_catalog(), JSON_UNESCAPED_SLASHES),
        'upload_limit_mb' => '5',
...
function admin_fetch_report_summary(array $filters = []): array
{
    ...
    $staffStmt = db()->prepare(
        'SELECT
         ...
         INNER JOIN issue_categories c ON c.id = i.category_id
         INNER JOIN users reporter ON reporter.id = i.user_id
         INNER JOIN users assignee ON assignee.id = i.assigned_to' . $whereSql . '
